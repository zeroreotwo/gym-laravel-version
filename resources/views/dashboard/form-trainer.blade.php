        ...
        <main class="main-content">
            ...
            <div class="glass-card" style="max-width: 900px; margin: 0 auto;">
                ...
                <div class="card-header" style="margin-bottom: 30px; border-bottom: 1px solid var(--glass-border); padding-bottom: 15px;">
                    <h2 class="card-title">{{ isset($trainer) ? 'Edit Paket Trainer' : 'Add New Paket Trainer' }}</h2>
                </div>

                <form action="{{ isset($trainer) ? url('dashboard/trainer/'.$trainer->id) : url('dashboard/trainer') }}" method="POST">
                    @csrf
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group-settings">
                            ...
                            <input type="text" name="nama" value="{{ old('nama', $trainer->nama ?? '') }}" required placeholder="Masukkan nama lengkap">
                        </div>

                        <div class="form-group-settings">
                            <label>Jumlah Hari Aktif</label>
                            <input type="text" name="hari" value="{{ old('tag', $trainer->hari ?? '') }}" required placeholder="">
                        </div>

                        <div class="form-group-settings">
                            <label>Harga</label>
                            <input type="text" name="harga" value="{{ old('harga', $trainer->harga ?? '') }}" required placeholder="Masukkan harga">
                        </div>
                        <div class="form-group-settings">
                            <label>Deskripsi</label>
                            <textarea name="detail" id="">{{ old('detail', $trainer->detail ?? '') }}</textarea>

                        </div>
                    </div>

                    <div class="btn-group" style="margin-top: 40px; justify-content: flex-end;">
                        <a href="{{ url('/dashboard/trainer') }}" class="btn btn-secondary" style="width: auto;">Cancel</a>
                        <button type="submit" class="btn btn-primary" style="width: auto; padding: 12px 30px;">
                            {{ isset($trainer) ? 'Update Paket Data' : 'Save New Paket Trainer' }}
                        </button>
                    </div>
