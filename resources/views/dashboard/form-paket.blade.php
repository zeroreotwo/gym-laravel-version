        ...
        <main class="main-content">
            ...
            <div class="glass-card" style="max-width: 900px; margin: 0 auto;">
                ...
                <div class="card-header" style="margin-bottom: 30px; border-bottom: 1px solid var(--glass-border); padding-bottom: 15px;">
                    <h2 class="card-title">{{ isset($paket) ? 'Edit Paket' : 'Add New Paket' }}</h2>
                </div>

                <form action="{{ isset($paket) ? url('dashboard/paket/'.$paket->id) : url('dashboard/paket') }}" method="POST">
                    @csrf
                    ...
                    <div class="form-grid">
                        ...
                        <div class="form-group-settings">
                            ...
                            <input type="text" name="nama" value="{{ old('nama', $paket->nama ?? '') }}" required placeholder="Masukkan nama lengkap">
                        </div>

                        <div class="form-group-settings">
                            <label>Tag</label>
                            <input type="text" name="tag" value="{{ old('tag', $paket->tag ?? '') }}" required placeholder="">
                        </div>

                        <div class="form-group-settings">
                            <label>Hari Aktif</label>
                            <input type="text" name="hari" value="{{ old('hari', $paket->hari ?? '') }}" required placeholder="">
                        </div>

                        <div class="form-group-settings">
                            <label>Harga</label>
                            <input type="text" name="harga" value="{{ old('harga', $paket->harga ?? '') }}" required placeholder="Masukkan harga">
                        </div>
                        <div class="form-group-settings">
                            <label>Deskripsi</label>
                            <textarea name="detail" id="">{{ old('detail', $paket->detail ?? '') }}</textarea>

                        </div>
                    </div>

                    <div class="btn-group" style="margin-top: 40px; justify-content: flex-end;">
                        <a href="{{ url('/dashboard/paket') }}" class="btn btn-secondary" style="width: auto;">Cancel</a>
                        <button type="submit" class="btn btn-primary" style="width: auto; padding: 12px 30px;">
                            {{ isset($paket) ? 'Update Paket Data' : 'Save New Paket' }}
                        </button>
                    </div>
