            ...
            <ul class="nav-menu">
                <li class="nav-section">
                    ...
                    <ul>
                        ...
                        <li class="nav-item">
                            <a href="{{ url('/dashboard/paket') }}" class="nav-link active">
                                <svg class="nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
            ...
            <section class="content-grid" style="grid-template-columns: 1fr;">
                <div class="glass-card table-card" style="grid-column: span 1;">
                    <div class="card-header" style="flex-wrap: wrap;">
                        <div class="card-title">paket Management</div>

                        <div class="card-actions" style="display: flex; gap: 12px; align-items: center; flex-wrap: wrap;">


                            <a href="{{ url('dashboard/paket/create') }}" class="btn btn-primary" style="padding: 10px 20px; width: auto; font-size: 14px;">
                                + Add paket
                            </a>
                        </div>
                    </div>
                    ...
                    <div class="table-wrapper">
                        <table class="data-table">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>
                                        <div class="table-paket">
                                            ...
                                            <div class="table-user-info">
                                                ...
                                                <span class="table-user-email">{{ $paket->tag }}</span>
                                            </div>
                                        </div>
                                    </td>
                                    ...
                                    <td>Rp. {{ $paket->detail }}</td>
                                    <td>{{ $paket->hari }} Hari</td>

                                    <td>
                                        <div style="display: flex ;">
                                            <a class="card-btn" href="/dashboard/paket/{{$paket->id}}/edit" style="padding: 6px 12px; margin-right: 1em;">Edit</a>
                                            <form action="/dashboard/paket/{{$paket->id}}" method="POST" id="form-delete-{{$paket->id}}">
                                                @csrf
                                                @method('DELETE')
                        ...
                        <div class="pagination-container">
                            <nav>
                                <ul class="pagination">
                                    <li class="page-item {{ $data_paket->onFirstPage() ? 'disabled' : '' }}">
                                        <a href="{{ $data_paket->url(1) }}" class="page-link">First</a>
                                    </li>

                                    <li class="page-item {{ $data_paket->onFirstPage() ? 'disabled' : '' }}">
                                        <a href="{{ $data_paket->previousPageUrl() }}" class="page-link">‹</a>
                                    </li>

                                    @php
                                    $current = $data_paket->currentPage();
                                    $last = $data_paket->lastPage();
                                    $start = max(1, $current - 2);
                                    $end = min($last, $current + 2);
                                    @endphp

                                    @if ($start > 1)
                                    <li class="page-item"><a href="{{ $data_paket->url(1) }}" class="page-link">1</a></li>
                                    @if ($start > 2)
                                    ...
                                    @for ($i = $start; $i <= $end; $i++)
                                        ...
                                        <a href="{{ $data_paket->url($i) }}" class="page-link">{{ $i }}</a>
                                        </li>
                                        @endfor
                                        ...
                                        @if ($end < $last)
                                            ...
                                            <li class="page-item disabled"><span class="page-link elipsis">…</span></li>
                                            @endif
                                            <li class="page-item"><a href="{{ $data_paket->url($last) }}" class="page-link">{{ $last }}</a></li>
                                            @endif

                                            <li class="page-item {{ !$data_paket->hasMorePages() ? 'disabled' : '' }}">
                                                <a href="{{ $data_paket->nextPageUrl() }}" class="page-link">›</a>
                                            </li>

                                            <li class="page-item {{ $current == $last ? 'disabled' : '' }}">
                                                <a href="{{ $data_paket->url($last) }}" class="page-link">Last</a>
                                            </li>
                                </ul>
                            </nav>
                        </div>
